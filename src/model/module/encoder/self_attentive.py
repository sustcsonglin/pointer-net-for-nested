...
class SelfAttentiveEncoder(nn.Module):
    def __init__(self, conf, input_dim, **kwargs):
        ...
        self.conf = conf
        # assert input_dim  == conf.d_model // 2, (f"input_dim:{input_dim}, d_model:{conf.d_model//2}")
        self.output_dim = conf.d_model
        self.input_dim = input_dim
        self.morpho_emb_dropout = FeatureDropout(conf.morpho_emb_dropout)
        self.add_timing = ConcatPositionalEncoding(
            d_model= conf.d_model,
        )
        encoder_layer = PartitionedTransformerEncoderLayer(
            conf.d_model,
            n_head=conf.num_heads,
            d_qkv=conf.d_kv,
            d_ff=conf.d_ff,
            ff_dropout=conf.relu_dropout,
            residual_dropout=conf.residual_dropout,
            attention_dropout=conf.attention_dropout,
        )
        self.encoder = PartitionedTransformerEncoder(
            encoder_layer, conf.num_layers
        )

        self.project_pretrained = nn.Linear(
            input_dim, self.conf.d_model // 2, bias=False
        )

    def forward(self, ctx):
        encoded_emb = ctx['embed']
        encoded_emb = [i for i in encoded_emb.values()]
        encoded_emb = self.project_pretrained(torch.cat(encoded_emb, dim=-1))

        b, seq_len = encoded_emb.shape[:2]
        lens = ctx['seq_len'] + 2
        mask = encoded_emb.new_tensor(range(seq_len)) < lens.view(-1, 1)
        h = self.add_timing(self.morpho_emb_dropout(encoded_emb))
        h = self.encoder(h, mask)
        # why? how important is this? tocheck::
        ctx['encoded_emb'] = h

        h = torch.cat([h[..., 0::2], h[..., 1::2]], -1)
        fencepost_annotations = torch.cat([h[:, :-1, : self.input_dim],
                                           h[:, 1:, self.input_dim: ]], dim=-1)
        ctx['fencepost'] = fencepost_annotations
