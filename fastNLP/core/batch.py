...
class DataSetGetter:
    ...
    """
    def __init__(self, dataset: DataSet, as_numpy=False, max_tokens=-1):
    ...
    def collate_fn(self, ins_list: list):
        ...
        if self.max_tokens != -1:
            seq_len =  sin_x['seq_len']
            costs = np.asarray(seq_len, dtype=int)
            costs_argsort = np.argsort(costs).tolist()
            subbatch_size = 1
            max_cost = self.max_tokens
            x = []
            y = []
            while costs_argsort:
                if subbatch_size == len(costs_argsort) or (
                    ...
                    subbatch_x = {name: [value[i] for i in subbatch_item_ids] for name, value in sin_x.items()}
                    subbatch_y = { name: [value[i] for i in subbatch_item_ids]  for name , value in sin_y.items()}
                    subbatch_x = _pad(subbatch_x, dataset=self.dataset, as_numpy=self.as_numpy)
                    subbatch_y = _pad(subbatch_y, dataset=self.dataset, as_numpy=self.as_numpy)
                    x.append(subbatch_x)
                    y.append(subbatch_y)
...
class DataSetIter(BatchIter):
    ...
    def __init__(self, dataset, batch_size=1, sampler=None, as_numpy=False, num_workers=0, pin_memory=False,
                 drop_last=False, timeout=0, worker_init_fn=None, batch_sampler=None, max_tokens=2500):
        r"""
