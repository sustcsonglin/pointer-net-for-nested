...
class BertEmbedding(nn.Module):
    ...
    def __init__(self, model, n_layers, n_out, stride=5, pad_index=0, dropout=0, requires_grad=False, from_pretrain=""):
        super().__init__()
        ...
        if from_pretrain == "":
            ...
            self.bert = AutoModel.from_pretrained(from_pretrain, config=AutoConfig.from_pretrained(from_pretrain,
...
class BertEmbedding_finetune(nn.Module):
    r"""
    A module that directly utilizes the pretrained models in `transformers`_ to produce BERT representations.
    While mainly tailored to provide input preparation and post-processing for the BERT model,
    it is also compatiable with other pretrained language models like XLNet, RoBERTa and ELECTRA, etc.
    ...
    def __init__(self, model, n_layers, n_out, stride=5, pad_index=0, dropout=0, requires_grad=False, from_pretrain=""):
        ...
        if from_pretrain == "":
            ...
            self.bert = AutoModel.from_pretrained(from_pretrain, config=AutoConfig.from_pretrained(from_pretrain, output_hidden_states=True))
        ...
        self.projection = nn.Linear(self.hidden_size, self.n_out, False)
            # if self.hidden_size != n_out else nn.Identity()
    ...
    def forward(self, subwords):
        ...
        bert = self.bert(subwords[:, :self.max_len], attention_mask=bert_mask[:, :self.max_len].float()).last_hidden_state
        ...
        for i in range(self.stride, (subwords.shape[1]-self.max_len+self.stride-1)//self.stride*self.stride+1, self.stride):
            part = self.bert(subwords[:, i:i+self.max_len], attention_mask=bert_mask[:, i:i+self.max_len].float()).last_hidden_state
            bert = torch.cat((bert, part, 1))
        ...
        # use the last token.
        embed = embed.gather(-2, bert_lens.unsqueeze(-1).unsqueeze(-1).expand(*embed.shape[:2], 1, embed.shape[-1])).squeeze(-2)
        embed = self.projection(embed)
        return embed
