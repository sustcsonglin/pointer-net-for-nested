...
class Field(RawField):
    ...
    def build(self, sequences, min_freq=1, embed=None):
        r"""
    ...
    def transform(self, sequences, tensorize=True, eos=True):
        r"""
...
class SubwordField(Field):
    ...
    def transform(self, sequences, tensorize=False, eos=True):
        ...
        if tensorize:
            ...
            sequences = [[ids[: i] for ids in seq] for i, seq in zip(lens, sequences)]
        return sequences


class ChartField_Banapar(Field):
    pass

class ChartField_Supar(Field):
    r"""
    ...
    def transform(self, charts, tensorize=True):
