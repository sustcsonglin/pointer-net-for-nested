...
class Tree(Transform):
    ...
    def build(cls, tree, sequence):
        ...
        def track(node):
            ...
            ## Representation of no-label:
            ## in supar: it ends with "|<>", in benepar, it uses <no_label>, in neural hpsg parser, it
            ## uses <no_label> and <no_label_H>

            if label.endswith('|<>') or label == '<no_label>' or label == '<no_label_H>':
                return children
